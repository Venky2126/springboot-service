...
/**
 * this class wil handle channel validation
 */
@Service
@AllArgsConstructor
@Slf4j
public class UserEncryptionChannel {

	private final KeyConfig keyConfig;
	private final ObjectMapper objectMapper;

	public EncryptedResponse encrypt(String channelId, Object customeResponse) {
		...
		try {
			...
			return this.encryption(channelId, requestPayload);
	...
	// encryption
	public EncryptedResponse encryption(String channelId, String plainRequest) {
		// JWE alg and Enc
		...
		try {
			...
			switch (channelId) {
			case "DVS":
				log.info("UserEncryptionChanneln validated channel channelId : {}", channelId);
				publicKey = keyConfig.getPublicKey();
				break;
			case "MMS":
				log.info("UserEncryptionChanneln validated channel channelId : {}", channelId);
				publicKey = keyConfig.getErrorPublicKey();
				break;

			default:
				throw new IllegalArgumentException("Unexcpected Value :" + channelId);
			}
